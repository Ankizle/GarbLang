...
var testers = []string{"manyclosurealloc", "loopalloc", "closurealloc"}
...
var builders = map[string]func(string){
	...
	"garb": func(name string) {
		...
		//compile to cc
		cmd1 := exec.Command("./garblang.bin", "garb/"+name+".garb", "garb/"+name+".cc")
		cmd1.Run()

		//compile the cc file in the garb/
		cmd3 := exec.Command("g++", "garb/"+name+".cc", "-o", "garb/"+name+".bin")
		cmd3.Run()
	},
	"go": func(name string) {
	...
	for _, v := range testers {
		...
		for i, k := range langOrder {
			...
			for t := 0; t < 300; t++ {
				...
				fmt.Println(real, user, sys, t, k, v)
